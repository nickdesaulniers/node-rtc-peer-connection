...
function Candidate (candidate) {
  ...
  this.component = 1;
  this.transport = 'UDP';
  this.priority = candidate.computePriority();
...
function Media (candidates) {
  ...
  for (var i = 0, len = candidates.length; i < len; ++i) {
    ...
    if (!seenSrflx && candidate.type === CandidateTypes.SERVER_REFLEXIVE) {
      seenSrflx = false;
      this.port = candidate.port;
      this.connection.ip = candidate.addr;
      this.connection.version = candidate.socket.address().family === 'IPv4' ? 4 : 6;
    }
  }
};
...
// returns a string representing a random 64b Integer.
var random64bInt = (function () {
  var twoToSixtyFour = (new BigNum(2)).toPower(64);
  return function () {
