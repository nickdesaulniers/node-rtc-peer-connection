...
IceAgent.prototype.addCandidate = function (candidate) {
  ...
  this.candidates.forEach(function (localCandidate) {
    ...
    // TODO: https://tools.ietf.org/html/draft-ietf-ice-trickle-01#section-8.1
    // mentions that the max length of the checklist should be 100.
  ...
  // Freeze all
  // TODO: does this conflict with trickle?
  // https://tools.ietf.org/html/draft-ietf-ice-rfc5245bis-00#section-5.1.3.4
  this.checklist.freezeAll();

  // Unfreeze the first
  this.checklist.unfreezeFirst();

  if (!this.checklist.isRunning()) {
    console.warn('not rerunning checks')
  }
  if (!this.checkTimeout && this.checklist.isRunning()) {
...
IceAgent.prototype.check = function () {
  ...
  if (pair) {
    ...
    if (pair) {
      ...
      // TODO: check if this.checklist is completed or failed
      // check length of this.checklist.validList
      return;
    }
  }
  this.checkTimeout = setTimeout(this.check.bind(this), this.Ta);
...
IceAgent.prototype.bindingSuccess = function (socket, packet, rinfo) {
  ...
  if (matchingSourceDest(rinfo, dest, candidatePair)) {
    ...
    // https://tools.ietf.org/html/draft-ietf-ice-rfc5245bis-00#section-6.2.1.3
    console.log('Found a peer reflexive candidate');
